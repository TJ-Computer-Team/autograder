...
async function check(user_data, req, res) {
  try {
    ...
    await cl.connect();
    let results = await cl.query("SELECT * FROM users WHERE id = ($1)", [
      user_data.id,
    ]);
    if (results.rows.length == 0) {
      ...
      req.session.tjioi =
        Math.floor(parseInt(req.session.userid) / 100) == 2024;
...
async function populate(user_data, req, res) {
  try {
    ...
    await cl.connect();
    await cl.query("INSERT INTO users VALUES ($1, $2, $3, $4, $5, $6, $7)", [
      user_data.id,
      user_data.display_name,
      user_data.ion_username,
      0,
      false,
      user_data.email,
      user_data.pass,

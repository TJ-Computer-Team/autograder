...
async function check(user_data, req, res) {
    try {
        ...
        let results = await pl.query("SELECT * FROM users WHERE id = ($1)", [user_data.id]);
...
async function populate(user_data, req, res) { 
    try {
        await pl.connect();
        await pl.query("INSERT INTO users VALUES ($1, $2, $3, $4, $5, $6)", [user_data.id, user_data.display_name, user_data.ion_username, false, user_data.email, user_data.pass]);
        await pl.end();
